 ...
 */
export async function showChatNotification(options: ChatNotificationOptions): Promise<void> {
  ...
  const { senderName, message, partTitle, senderImage, conversationId } = options;

  const notificationTitle = `${senderName} sent you a message`;
  const notificationBody = partTitle
    ? `Re: ${partTitle}\n${message}`
    : message;
