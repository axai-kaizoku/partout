...
export const partRouter = createTRPCRouter({
  ...
  _getSearchResults: publicProcedure.input(z.object({
    ...
    if (search) {
      searchQuery = {
        ...
        description: (description, { ilike }) => ilike(description, `%${search}%`),
      }
    }
    ...
    if (filters) {
      filtersQuery = {
        categoryId: (categoryId, { eq }) => eq(categoryId, filters.categoryId),
        ...
        priceRange: (priceRange, { gte, lte }) => gte(priceRange, filters.priceRange?.[0]) && lte(priceRange, filters.priceRange?.[1]),
        location: (location, { eq }) => eq(location, filters.location),
        negotiable: (negotiable, { eq }) => eq(negotiable, filters.negotiable),
      }
    }
  ...
  getSearchResults: publicProcedure
    ...
    .query(async ({ input }) => {
        ...
        with: {
          ...
          partCompatibility: {
            where: (compat, { and, eq, gte, lte }) => {
              ...
              return compatConditions.length > 0 ? and(...compatConditions) : undefined;
            },
            with: {
              make: {
                columns: {
      ...
      const filteredParts = (makeId || modelId || filters?.year)
