...
import {
  basicInfoDefaults,
  priceShippingDefaults,
  steps,
  vehicleDetailsDefaults,
} from "./form-defaults";
import { PricingShippingForm } from "./pricing-shipping-form";
import {
  basicInfoSchema,
  pricingShippingSchema,
...
export function NewListingForm() {
  ...
  const { mutateAsync: createModelForMake } =
    api.partInfo.createModelForMake.useMutation();
  ...
  const { mutateAsync: createPartImage } =
    api.image.createPartImage.useMutation();

  const { mutateAsync: createPartShipping } =
    api.part.createPartShipping.useMutation();
  ...
  const handleFileSelect = async (
    ...
    for (const file of Array.from(files)) {
      ...
      if (
        ...
        images.length >= 8 ||
        Array.from(files).length + images.length > 8
      ) {
        toast.error("You can only upload up to 8 images.");
        return;
      }
      ...
      photoForm.setFieldValue("photos", [
        ...photoForm.state.values.photos,
        imageId,
      ]);
  ...
  const removeImage = (imageId: string) => {
    const filtered = photoForm.state.values.photos.filter(
      (id) => id !== imageId,
    );
        ...
        <div className="flex items-center justify-between">
          {steps.map((step, index) => (
            <CurrentStepIndicator
              ...
              index={index}
              className={index + 1 !== currentStep ? "hidden md:flex" : ""}
            />
          ))}
        </div>
      </div>
      ...
      <form id="partform" onSubmit={handleSubmit}>
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <LoadingButton
              disabled={pricingShippingForm.state.isSubmitting}
